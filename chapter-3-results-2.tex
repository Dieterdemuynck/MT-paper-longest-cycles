...
Similar to Chapter~\ref{cha:results}, in this chapter, we discuss the correctness tests performed and their results on the algorithm designed for research question \ref{rq2}, as well as the results of running the two different pipelines from Section~\ref{sec:generators-used} and the observations gained from them. We also shortly discuss the time each pipeline took for each of the parameters, and how the list of non-Hamiltonian planar graphs with minimum degree at least 4 greatly improved the maximum order we can make definitive claims on. Lastly, we find and prove a new family of graphs which are negative examples of the statement in RQ\ref{rq2}, that are discovered by researching the gadget discussed in Section~\ref{sec:gadget}.
...
\section{Conclusion}
We presented the computational and theoretical results obtained for RQ\ref{rq2}. We exhaustively searched all 2-connected planar graphs of minimum degree at least 4 generated by plantri, as well as a supplied list of only those graphs that could hold negative examples of the question, extending our coverage to graphs of order 20. Across every graph checked, no negative example of RQ2 was found: whenever every vertex-deleted subgraph contains a $k$-cycle, the graph itself contains a longer cycle. This indicates that any further negative examples of RQ2 beyond those already known must occur at orders larger than we could search exhaustively.

We also generalized the gadget underlying Zamfirescu's construction $G_{p,\ell}$ by additionally inserting vertices on the edges of the outer cycles, giving a new family $G_{p,\ell,D}$ parametrized by an ordered set of integers $D$. We showed that, subject to the condition $\sigma \le (p-3)\ell + 2\min(D)$, this family is 2-connected with circumference $(p-1)\ell + \sigma$ and empty longest-cycle intersection. For $\ell \ge 5$ and $d_i = 1$ or $d_i \ge 5$, with at least one $d_i \ge 5$, this yields new negative examples of RQ\ref{rq2} beyond the graphs obtained from Zamfirescu's construction.
